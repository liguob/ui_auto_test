...
class PostInfoPage(BasePage):
    ...
    def edit_info(self, values: dict, name: str = ''):
        ...
        keys = values.keys()
        if "职务名称" in keys:
            self.locator_text_input(ctrl_id='job', value=values["职务名称"])
        if "自定义职务" in keys:
            self.locator_text_input(ctrl_id='custom_job', value=values["自定义职务"])
        if "职务级别" in keys:
            self.locator_select_list_value(ctrl_id='level', value=values["职务级别"])
        if "评定日期" in keys:
