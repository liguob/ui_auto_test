...
class TrainingManagePage(PersonnelSysPage):
    """培养管理主页面"""

    @allure.step("新增培养信息")
    def add_training(self, values: dict, user_name=''):
        self.locator_tag_button(button_title='新增')
        self.wait_open_new_browser_and_switch()
        from djw.page.smart_school_sys.人事系统.人员管理.培养管理.training_info import TrainingInfoPage
        TrainingInfoPage(self.driver).edit_info(name=user_name, values=values)
        return self
    ...
    def edit_training(self, name, values: dict):
        ...
        time.sleep(2)  # 等待数据加载
        from djw.page.smart_school_sys.人事系统.人员管理.培养管理.training_info import TrainingInfoPage
        TrainingInfoPage(self.driver).edit_info(values=values)
        return self

    @allure.step("删除单个培养信息")
    ...
    @allure.step("查询培养信息")
    def search_training(self, value, placeholder: str = '部门/姓名'):
        self.locator_tag_search_input(placeholder=placeholder, value=value, enter=True)
    ...
    def download_model(self):
        ...
        self.locator_dialog_btn(btn_name='培养信息导入模板.xls')
        return wait_file_down_and_clean(file_name='培养信息导入模板.xls')

    @allure.step("培养信息导出")
    def download_info(self):
        self.locator_more_tip_button(button_title='导出')
        self.locator_dialog_btn(btn_name='导出', dialog_title='导出设置', need_close=True)
        return wait_file_down_and_clean(file_name='培养信息.xlsx')
